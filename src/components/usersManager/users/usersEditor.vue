      ...
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12>
                <v-text-field label="Legal first name*" required :value="userGroup.name"></v-text-field>
              </v-flex>

              <v-flex xs12>
                <v-text-field label="Email*" required :value="userGroup.mail"></v-text-field>
              </v-flex>

              <v-flex xs12>
                <v-switch :key="group ._id  " :label="group .name" v-for="group in userGroup"></v-switch>
              </v-flex>
            </v-layout>
          </v-container>
    ...
    edictUserGroup: function() {
      ...
      if (this.groupId) {
        ...
        this.$http
          ...
          .then(
            response => (this.userGroup = response.data.items),
            (this.userEditordialog = true)
          )
        ...
        this.$emit("update:userEditordialog", this.userEditordialog);
